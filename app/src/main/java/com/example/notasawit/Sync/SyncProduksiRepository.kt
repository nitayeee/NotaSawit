...
class SyncProduksiRepository(
    ...
    suspend fun syncProduksi(): Boolean = withContext(Dispatchers.IO) {
        if (!syncMutex.tryLock()) {
            Log.d("SYNC_PRODUKSI", "Sinkronisasi produksi sedang berjalan di thread lain, skip.")
            return@withContext true
        }

        try {
            // 1. Ambil data produksi lokal yang belum tersinkron (isSynced = false)
            val produksiList = database.ProduksiDao().getUnsynced()
            ...
            produksiList.forEach { produksi ->
                ...
                val detailLahan = database.DetailProduksiDao().getByProduksi(produksi.localId)
